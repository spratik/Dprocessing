...
public:
	...
	void ProcessImage (Mat & ,int ,int ,int ,char * ,char *);
...
{
	if(argc != 4){
		cout << "EXE image BB.pbm CC.pbm" << endl;
		return 0;
	}
	int rs = Success;
	//! read the image
	...
	obj.ProcessImage(image, size , rows , cols ,argv[3] ,argv[2]);
...
void BoundingBox (ConnectedComponent *obj ,const int cols ,const int rows ,char *bounding_box_file)
{
	...
	dumpDataIntoFile (obj ,size ,cols ,rows ,bounding_box_file);
...
void ConnectedComponent::ProcessImage (Mat &image ,int size ,int rows ,int cols ,char *bounding_box_file ,char *connected_component_file)
{
